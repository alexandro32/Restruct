...
Процедура ИнициализироватьПеременные()
	...
	//Добавить параметры АдресСервера и АдресБазы, тогда можно
	
	Если СтрНайти(ТекущийКаталог(),"_corp") Тогда
	 	ПараметрыОбработки.Вставить("ПутьПапкиВыполнения", "D:\restruct_corp\");
		ПараметрыОбработки.Вставить("ПутьКБазе","er124-db\UserBaseCorp");                                // $PathBase
		//ПараметрыОбработки.Вставить("ПутьКБазе", "er124-db\UserBasePBOUL"); 
		ПараметрыОбработки.Вставить("БазаРеструктуризации", "er124-db\ea_restruct_corp");                   // $RestructBase
