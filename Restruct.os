...
Процедура УдалитьУстаревшиеФайлы(Файлы, ПутьКфайлу)
...
Процедура ВыполнитьКоманду(Параметр)
...
Процедура ПроверитьРеструктуризацию()
	...
	ВременныйЛогФайл = СтрШаблон("%1%2_%3.txt",ПараметрыОбработки.ПутьПапкиВыполнения,ПараметрыОбработки.ЛогФайл, ПараметрыОбработки.ТекущаяДата);
	//ПараметрыОбработки.ЛогФайл = ВременныйЛогФайл;
	
	ЛогФайл = Новый ТекстовыйДокумент;
	ЛогФайл.Записать(ВременныйЛогФайл);
...
Процедура ПроверитьНаличиеФайлов()
	...
	СписокФайловОбработки = ПолучитьСписокФайловОбработки();
...
Процедура ОбработатьФайлыДанных()
	...
	Для Каждого КлючИЗначение Из СписокФайловОбработки Цикл
		КопироватьФайлыВАрхив(КлючИЗначение.Ключ,КлючИЗначение.Значение);
		УдалитьУстаревшиеФайлы(КлючИЗначение.Ключ,КлючИЗначение.Значение);
	КонецЦикла;
		
	УдалитьУстаревшиеФайлы(ПараметрыОбработки.ФайлКопииБазы, ПараметрыОбработки.ПутьПапкиВыполнения);
	
	//ПараметрыОбработки.OUT = OUT;
	
КонецПроцедуры

Функция ПолучитьСписокФайловОбработки()
...
Процедура КопироватьФайлыВАрхив(Файлы, ПутьКфайлу)
